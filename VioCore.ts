...
export {
  AnimationComponent,
  ObjectBindings,
} from "./VioSource/Components/AnimationComponent";
...
class FileCache {
  ...
  public static get instance(): FileCache {
    if (FileCache._instance == null) FileCache._instance = new FileCache();
...
THREE.FileLoader.prototype.load = function (url, onLoad, onProgress, onError) {
  ...
  else FileLoaderLoad.call(this, url, onLoad, onProgress, onError);
};
...
THREE.ImageLoader.prototype.load = function (url, onLoad, onProgress, onError) {
  ...
  if (window.location.hostname == "localhost") {
    let prefix = "https://app.cinevva.com/";
    if (url.startsWith(prefix + "sfx")) url = url.slice(prefix.length - 1);
  }
...
THREE.TextureLoader.prototype.load = function (
  ...
  else return TextureLoaderLoad.call(this, url, onLoad, onProgress, onError);
};
...
THREE.PropertyBinding.findNode = (root, nodeName) => {
  ...
  if (!target) {
    if (nodeName == "root" || nodeName == "Root") target = root;
  }
  return target;
};
...
let setAnimationClipDataData = async (clip: THREE.AnimationClip, data: any) => {
  ...
  let buffers = new Float32Array(
    (await fetch(data.buffers).then((res) => res.arrayBuffer())) as ArrayBuffer
  );
  // console.log(buffers)
  clip.tracks = [];
...
export { getAnimationClipData, setAnimationClipDataData };
