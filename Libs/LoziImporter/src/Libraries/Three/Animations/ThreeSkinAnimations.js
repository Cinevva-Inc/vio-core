...
export default class ThreeSkinAnimations
{
    static setAnimationData(obj,data)
    {
        if(data && obj)
        {
            ...
            if(animData)
            {
                ...
                var anim    = this.generateTransformAnimation(obj,animData);
    ...
    static generateTransformAnimation(obj,anim)
    {
        if(anim.clips)
        {
            ...
            {
                // mixer[anim.clips[num].name] = this.generateTransformAnimationClip(obj,anim.clips[num]);
                // mixer.clipAction(mixer[anim.clips[num].name]);
                generated.clips.push(this.generateTransformAnimationClip(obj,anim.clips[num]));
            }
            return generated;
            // // create a ClipAction and set it to play
            // var clipAction = window['mix2'].clipAction( generatedClip );
            // window['ac'] = clipAction;
        }
    }
    ...
    static generateTransformAnimationClip(obj,clip)
    {
        var arr = [];
        for(var num = 0; num < clip.hierarchy.length; num++)
        {
            var index         = LoziUtils.getObjectIndex(obj.objects,'name',clip.hierarchy[num].name);
