...
export class VioGenerator {
  public static async generateAsset(
    ...
    switch (data.type) {
      ...
      case "object": {
        generated = await VioGenerator.generateObject(
          data,
          (total: number, count: number, status) => {
            if (onProgress) {
  ...
  public static async loadProject(
    ...
    if (resp.data && resp.data.type == "project") {
      ...
      switch (sceneData.type) {
        case "inline": {
          VioRender.scene = await this.generateScene(
            sceneData.object,
            onProgress,
            onComplete
          );
  ...
  public static async loadScene(
    ...
    if (resp.data && resp.data.type == "scene") {
      scene = await this.generateScene(
        resp.data,
        onProgress,
        onComplete,
        onScene
  ...
  public static async loadObject(
    url: string,
    onProgress?: (
      total: number,
      progress: number,
    ...
    if (data.objects) {
      ...
      for (let num = 0; num < data.objects.length; num++) {
        if (data.objects[num].bounds) {
          min.set(
            data.objects[num].bounds.min.x,
            data.objects[num].bounds.min.y,
            data.objects[num].bounds.min.z
          );
          max.set(
            data.objects[num].bounds.max.x,
            data.objects[num].bounds.max.y,
            data.objects[num].bounds.max.z
          );
          box.set(min, max);
          ...
          if (
            VioRender.camera.frustum.containsPoint(data.objects[num].position)
          ) {
            obj.frustum.push(data.objects[num]);
          } else {
            obj.objects.push(data.objects[num]);
  ...
  private static async loadObjects(
    ...
    await Promise.all(
      objects.map(async (object: any, num: number) => {
        let instance = await VioGenerator.generateObject(
          objects[num],
          (objectTotal: number, progress: number, status: string) => {
            if (onProgress) {
              onProgress(
                progressIndex,
                num,
                progress / objectTotal,
                objects.length
              );
            }
          }
    ...
    if (data.objects) {
      ...
      let onProgressCB = (
        ...
        let canRender: boolean =
          Math.round(prog[0].reduce((a, b) => a + b, 0)) == 1;
  ...
  public static async generateScene4(
    data: any,
      ...
      for (let num = 0; num < objects.frustum.length; num++) {
        ...
        let instance = await VioGenerator.generateObject(
          ...
          (objectTotal: number, progress: number, status: string) => {
            ...
            overallProgress =
              (prog[0].reduce((a, b) => a + b, 0) +
                prog[1].reduce((a, b) => a + b, 0)) /
              objects.count;
            if (onProgress) {
  ...
  public static async generateScene3(
    data: any,
    onProgress?: (progress: number, status: string) => void | null
  ) {
        ...
        setTimeout(async () => {
          ...
          let instance = await VioGenerator.generateObject(
            ...
            (objectTotal: number, progress: number, status: string) => {
              ...
              overallProgress =
                prog.reduce((a, b) => a + b) / data.objects.length;
              if (onProgress) {
                onProgress(overallProgress, status);
              }
            }
  ...
  public static async generateScene2(
    data: any,
    onProgress?: (progress: number, status: string) => void | null
  ) {
    let total = this.getOverallObjects(data);
  ...
  public static async generateObject(
    data: any,
    onProgress?: (
      total: number,
      progress: number,
    ...
    let meta = data.meta ?? {};
    if (meta.icon?.startsWith("blob:")) delete meta.icon;
    ...
    if (data.objects) {
      (
        await Promise.all(
          data.objects.map((nestedData: any) =>
            VioGenerator.generateObject(nestedData)
          )
        )
      ).forEach((nestedObject: VioObject) => obj.addObject(nestedObject));
    }
    ...
    if (data.components) {
      ...
      for (let num = 0; num < data.components.length; num++) {
        ...
        if (comp) obj.addComponent(comp);
      }

      for (let num = 0; num < obj.components.length; num++) {
        (window as any).component = obj.components[num];
