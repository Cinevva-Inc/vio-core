...
export class VioScene extends Scene {
  ...
  public static async load(
    ...
    onScene: (scene: VioScene) => void | null
  ) {
  ...
  public getObjectByObjectID(objectID: string): VioObject | null {
    ...
    this.traverse((object: any) => {
      if (object.objectID == objectID) foundObject = object as VioObject;
    });
    return foundObject;
  }
  ...
  public update(delta: number): void {
    if (this.skybox.object) {
      let scale = VioRender.camera.cameraFrustumDistance * 0.9;
    ...
    for (let object of this.objects) {
      if (object.visible) object.update(delta);
    }
  }
  ...
  public async setData(data: any) {
    ...
    if (data.objects) {
      await Promise.all(
        data.objects.map(async (data: any) => {
          try {
    ...
    if (data.lights) {
      ...
      this.ambientLight.copy(
        loader.parse(data.lights.ambient) as THREE.AmbientLight
      );
      this.hemisphereLight.copy(
        loader.parse(data.lights.hemisphere) as THREE.HemisphereLight
      );
      this.directionalLight.copy(
        loader.parse(data.lights.directional) as THREE.DirectionalLight
      );
    } else {
      if (data.ambient) {
