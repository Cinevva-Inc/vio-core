...
export class VioInput {
  ...
  private constructor() {
    ...
    VioRender.canvasElement.addEventListener(
      "wheel",
  ...
  _onTouch(e: TouchEvent) {
    ...
    let oldIdentifier = this._pointerBuffer
      ? this._pointerBuffer.identifier
      : -1;
    ...
    this._pointerBuffer!.type =
      e.type == "touchmove" ? "move" : e.type == "touchstart" ? "down" : "up";
  ...
  _onMouse(e: MouseEvent) {
    // console.log("_onMouse", e.type, e.button, e.clientX, e.clientY);
    if (VioInput.isTouchEnabled) {
      return;
    }

    let button = VioEditMode.enabled
      ? e.type == "mousemove"
        ? this._pointerBuffer
          ? this._pointerBuffer.button
          : -1
    ...
    this._pointerBuffer!.type =
      e.type == "mousemove" ? "move" : e.type == "mousedown" ? "down" : "up";
  ...
  onPointer(e: PointerData) {
    // console.log("onPointer", e.type, e.button, e.position.x, e.position.y);
  ...
  public static removeBinding(keyWord: string, key: string) {
    if (this.instance._bindings.has(keyWord)) {
      const index: number = this.instance._bindings
        .get(keyWord)!
  ...
  public static registerKeyEvent(
    listener: (type: string, keys: Array<string>) => void
  ) {
  ...
  public static unregisterKeyEvent(
    listener: (type: string, keys: Array<string>) => void
  ) {
  ...
  public static getPositionByCursor(
    pointX: number,
    pointY: number
  ): { position: Vector; normalized: Vector; viewport: Vector } {
    ...
    this.instance._bufferedPosition.normalized.x =
      this.instance._bufferedPosition.position.x / rect.width;
    this.instance._bufferedPosition.normalized.y =
      this.instance._bufferedPosition.position.y / rect.height;

    this.instance._bufferedPosition.viewport.x =
      this.instance._bufferedPosition.normalized.x * 2 - 1;
    this.instance._bufferedPosition.viewport.y =
      -this.instance._bufferedPosition.normalized.y * 2 + 1;
