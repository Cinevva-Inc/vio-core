...
export class VioRender {
  ...
  private constructor() {
    ...
    if (isMobileDevice) this._interval = 1 / 30;
    ...
    this._offScreen =
      !isIOS && (this._canvas as any).transferControlToOffscreen
        ? (this._canvas as any).transferControlToOffscreen()
        : null;
    ...
    this._rendererPass = new RenderPass(
      this._rootScene,
      this._camera.selectedCamera
    );
  ...
  public render2(delta: number) {
    ...
    this._renderer!.info.reset();
    if (this._composer!.passes.length > 0) this._composer!.render(delta);
    else this._renderer!.render(this._rootScene, this._camera.selectedCamera);
    VioInput.update();
  }
  ...
  public static set clearColor(colorHex: string) {
    let hex = parseInt(
      colorHex[0] == "#" ? colorHex.slice(1, 7) : colorHex,
      16
    );

    this.instance._clearColor.setHex(hex);
    this.instance._renderer!.setClearColor(
      this.instance._clearColor,
      this.instance._alpha
    );
  }
  ...
  public static set renderPasses(passes: Array<Pass>) {
    ...
    if (passes.length > 0) {
      ...
      this.instance._rendererPass.setSize(
  ...
  public static addRenderPass(pass: Pass) {
    if (VioRender.renderPasses.length == 0) VioRender.renderPasses = [pass];
    else VioRender.renderPasses.push(pass);
  }

  public static removeRenderPass(pass: Pass) {
    let i = VioRender.renderPasses.indexOf(pass);
    if (i != -1) VioRender.renderPasses.splice(i, 1);
  }
