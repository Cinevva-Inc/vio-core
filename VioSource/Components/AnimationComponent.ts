...
export class AnimationComponent extends VioComponent {
  ...
  private _replaceAction(
    ...
    console.log("_replaceAction", animation.name, action);
    ...
    if (action) {
      ...
      if (animation.action?.pending !== undefined)
        animation.action.pending = false;
    }
  }
  ...
  public async setData(data: any) {
    ...
    for (let animation of data.animations ?? []) {
      if (animation.name && animation.source) {
        ...
        let i = this._animations.findIndex(
          ({ name }) => animation.name == name
        );
        if (i != -1) this._animations.splice(i, 1);
        this._animations.push(animation);
        this.loadAction(animation.source).then((action) =>
          this._replaceAction(animation, action)
        );
      }
    }
    super.setData(data);
  ...
  public play(name: string, exclusive: boolean = false) {
    for (let animation of this._animations) {
      if (animation.name == name) {
        ...
        this._boundingBoxUpdateRequested = true;
      } else if (exclusive) animation.action!.stop();
    }
  }

  public stop(name: string) {
    for (let animation of this._animations)
      if (animation.name == name) animation.action!.stop();
  }
  ...
  public addAnimation(
    ...
    this._animations.push(animation);
    this.loadAction(source).then((action) =>
      this._replaceAction(animation, action)
    );
  }
  ...
  public removeAnimation(name: string) {
    ...
    if (i != -1) {
      ...
      if (animation.action && animation.source.url) {
        ...
        if (clip) this._meshComponent!.animationMixer!.uncacheAction(clip);
      }
  ...
  public renameAnimation(oldName: string, newName: string) {
    let animation = this._animations.find(
      (animation) => animation.name == oldName
    );
    if (animation) animation.name = newName;
  }

  private loadAction(source: {
    url: string | null;
    clip: string;
    json: any;
    root: Object3D | null;
    ...
    console.log("loadAction 1", source);
    return new Promise((resolve, reject) => {
      let meshComponent = this._meshComponent;
      if (meshComponent) {
        console.log("loadAction 2", source);
        meshComponent.isInitialized().then(async () => {
          console.log("loadAction 3", source);
          try {
            if (source.json) {
              let clip = new AnimationClip("", 0);
              console.log("loadAction 3.1", source);
              await setAnimationClipDataData(clip, source.json);
              console.log("loadAction 3.2", source);
              resolve(
                meshComponent.animationMixer!.clipAction(
                  clip,
                  source.root ?? meshComponent.meshObject!
                )
              );
              console.log("loadAction 3.3", source);
            } else if (source.url == null) {
              resolve((meshComponent.animations ?? {})[source.clip]);
            } else {
              let clip = await VioResources.getAnimation(
                source.url,
                source.clip,
                source.json,
                source.bindings ?? ObjectBindings
              );
              if (clip)
                resolve(
                  meshComponent.animationMixer!.clipAction(
            ...
            console.error("Error loading animation action:", err);
            reject(err.message);
          }
        });
        console.log("loadAction 4", source);
      } else resolve(null);
    });
  }
  ...
  set object(object: VioObject | null) {
    if (this.object) this.onRemoved();
    super.object = object;
    if (this.object) this.onAdded(this.object);
  }

  onAdded(object: VioObject) {
    this._meshComponent = object.getComponentByType(
      MeshComponent
    ) as MeshComponent;
    if (this._meshComponent) {
      this._meshComponent.isInitialized().then(() => {
        for (let [name, action] of Object.entries(
          this._meshComponent!.animations ?? {}
        ))
          if (
            !this._animations.find(
              (animation) =>
                animation.source.url == null && animation.source.clip == name
            )
          ) {
            let i = this._animations.findIndex(
