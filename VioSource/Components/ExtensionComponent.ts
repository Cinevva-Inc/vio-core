    ...
    class Component extends VioComponent {
      ...
      public setData(data: any): void {
        ...
        if (this.active) {
          if (this.funcs.tune) {
            ...
            if (this.funcs.term) this.funcs.term();
            if (this.funcs.init) this.funcs.init();
          }
        }
      }
      public get missingDeps(): Array<any> {
        return this.deps.filter(
          (dep: any) =>
            this.object &&
            !this.object!.getComponent(dep.uuid) &&
            !this.object!.getComponent(dep.load) &&
            !(window as any).app.extensions.activeEffects.includes(dep.uuid)
        );
      }
      ...
      set object(object: VioObject | null) {
        ...
        if (this.object) {
          if (this.depsSatisified) {
            if (this.funcs.init) this.funcs.init(object);
            this.active = true;
          }
        }
      }
      ...
      update(delta: number) {
        if (this.active && this.inited) {
          if (this.funcs.tick) this.funcs.tick(delta);
        }
      }
      tune() {
        if (this.active) {
          if (this.funcs.tune) this.funcs.tune();
        }
      }
      step() {
        if (this.active && this.inited) {
          if (this.funcs.step) this.funcs.step();
        }
      }
    }
